...
**Note:** Tokens are only required for the HTTP API. If you're using Native Messaging with the MCP server, no tokens are needed.

If you want to use the HTTP API:

1. **Generate an access token:**
   - Tokens are no longer generated by the extension
   - Create your own secure random token (see ACCESS_CONTROL_SETUP.md)
...
- ✅ Manifest-based security (Chrome validates extension ID)
...
See [ACCESS_CONTROL_SETUP.md](ACCESS_CONTROL_SETUP.md) for HTTP API security best practices.
...
- Verify token is correct (check your secure storage)
...
- See [ACCESS_CONTROL_SETUP.md](ACCESS_CONTROL_SETUP.md) for token generation
